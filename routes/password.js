...
router.post("/recover", async (req, res) => {
  ...
  try {
    ...
    const link = "https://" + req.header.host + "/resetpassword/reset/" + user.resetPasswordToken;
...
router.get("/reset/:token", (req, res) => {
  User.findOne({
    ...
    .then(user => {
      ...
      //Redirect user
      res.send(
        `send post request to http://localhost:3000/resetpassword/reset/${req.params.token} with body object 'password': 'New Password' `
      );
    })
    .catch(err => res.status(500).json({ message: err.message }));
});
